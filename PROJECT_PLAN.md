...
```json
{
  ...
  "master_prompt_path": "prompts/master_history.txt",
  "llm": {
    "presets": {
...
```
videoforge/
├── .cursorrules / .claudeignore / .gitignore
...
├── prompts/
│   ├── master_script_v1.txt      # Мастер-промпт (вже є, вставити сюди)
...
- Вибір моделі через параметр. Async httpx. Retry. Cost tracking per model.
...
- CLI: `python modules/01_script_generator.py --source "D:/transscript batch/Transcriber/output/Video Name" --channel config/channels/history.json`
- Залежить від: 2

**№7 — Image Generator** (modules/02_image_generator.py)
- script.json → WaveSpeed паралельно (asyncio.gather) → images/
...
- `--quality max|high|balanced|bulk|test` — вибір preset'у моделей з channel config (default: max)
- --from-step, --model (override конкретну модель)
- Залежить від: 15
