...
def _scan_ready_projects() -> list[dict[str, Any]]:
    ...
    for p in sorted(PROJECTS.iterdir()):
        ...
        results.append({
            ...
            "has_thumbnail": (out / "thumbnail.png").exists() or (out / "thumbnail.jpg").exists(),
...
async def youtube_upload(req: UploadRequest) -> dict[str, Any]:
    ...
    # Module 08 uses script_path.parent as project base → project_dir/output/final.mp4
    # We pass project_dir/"script.json" so base_dir = project_dir, out_dir = project_dir/output
    script_json = project_dir / "script.json"
    if not (project_dir / "output" / "final.mp4").exists():
        raise HTTPException(400, f"final.mp4 not found in {project_dir}/output/")
    ...
    async def _run() -> None:
        ...
        try:
            # Module name starts with digit — use spec_from_file_location
